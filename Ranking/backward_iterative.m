...
val_det_back = zeros(1,max_k); % list of sub-determinant values after each exclusion
identity_det_back = zeros(1,max_k); % list of sub-determinant index chosen
list = 1:max_k; 

while k>=n
    adjX = inv(SsX) * det(SsX); % compute adjoint of matrix
    detval = diag(adjX); % compute sub-determinant values
    [maxval,argmax] = max(detval(:)); % choose the max
    val_det_back(1,max_k-k+1) = maxval;
    SsX(argmax,:) = []; % remove column entries for rejected index
    SsX(:,argmax) = []; % remove row enties for rejected index
...
disp('Iterative Backward Approach');
disp('Entries with decresing order of importance:');
disp(identity_det_back);
disp('Corresponding sub-determinant value:');
disp(num2str(val_det_back(:).'))
